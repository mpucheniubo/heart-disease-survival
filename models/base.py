...
class Base:
	...
	class Decorators:
		...
		def verified(cls, func: Callable[[Any], Any]) -> Callable[[Any], Any]:
			def decorated(cls, *args, **kwargs):
				logging.info(f"Applying decorator verified to method '{func.__name__}'.")
				value = None
				if cls:
	...
	def set_booleans(self) -> None:
		...
		for column in self.data.select_dtypes(include="object"):
			if self.data[column].nunique() == 2 and column not in self.primary_key:
				...
				self.data[column] = self.data[column].map(self.boolean_mapping[column]).astype("bool")
	...
	def set_categories(self) -> None:
		...
		"""
		logging.info("Setting columns with more than 2 different entries to 'category'.")
		for column in self.data.select_dtypes(include="object"):
			if self.data[column].nunique() > 2 and column not in self.primary_key:
